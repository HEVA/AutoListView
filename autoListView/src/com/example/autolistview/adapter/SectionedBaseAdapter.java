...
public abstract class SectionedBaseAdapter extends BaseAdapter implements PinnedSectionedHeaderAdapter {
	...
	/**
	 * 类似ExpandablelistView 通过组的位置和无素的位置来获得Item
	 */
	@Override
	...
	/**
	 * 给每个item标记好group的值
	 * position是在ListView中的位置 i是每个Setion分组的位置
	 */
	...
	public int getPositionInSectionForPosition(int position) {
		...
		for (int i = 0; i < internalGetSectionCount(); i++) {
			...
			if (position >= sectionStart && position < sectionEnd) {
				...
				currentSection = positionInSection;
